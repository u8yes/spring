...
return reVal;
	...
	public String midFind(UserMemberVO user , HttpSession session) {
		
		if(user.getUm_ars() == null || user.getUm_ars().equals("")) {
			throw new NullPointerException("이름은 반드시 입력해야합니다.");
		}
		
		if(user.getUm_email() == null || user.getUm_email().equals("")) {
			throw new NullPointerException("이메일은 반드시 입력해야합니다.");
		}	
...
return reVal;
	...
	public String mpsFind(UserMemberVO user , HttpSession session) {
