	...
	<center>
		...
		<table border="1" cellpadding="0" cellspacing="0" width="800" class="table table-striped table-sm text-center">
			<tr>
				...
				<th bgcolor="orange" width="100">로고 이미지</th>
			</tr>
			<c:forEach items="${brandThemeList}" var="brandTheme">	<!-- 1. JSTL 기능 중 items는 데이터, 2. var는 새 변수 선언 -->
			<tr>
				<td><a href="getBrandTheme.bt?bt_no=${brandTheme.bt_no}">${brandTheme.bt_no}</a></td>	<!-- ?seq= 라는 건 바로 쿼리문으로 연결되는 것 -->
				<td align="center">${brandTheme.bm_no}</td>
  ...
  <div class="album py-5 bg-light">
    <div class="container">
      ...
      <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
        <c:forEach items="${brandThemeList}" var="brandTheme">
        <div class="col">
          <div class="card shadow-sm">
            <svg class="bd-placeholder-img card-img-top" width="100%" height="225" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Placeholder: Thumbnail" preserveAspectRatio="xMidYMid slice" focusable="false">
            <image href="./brandUpload/${brandTheme.bt_logo}" width="100%"></image>
            <title>Placeholder</title></svg>

            <div class="card-body">
              <p class="card-text">${brandTheme.bm_no}</p>
              <p class="card-text">${brandTheme.bt_no}</p>
              <div class="d-flex justify-content-between align-items-center">
                <div class="btn-group">
                  <button type="button" class="btn btn-sm btn-outline-secondary"><a href="getBrandTheme.bt?bt_no=${brandTheme.bt_no}">View</a></button>
                </div>
